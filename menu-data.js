// menu-data.js — مطعم المهدى للمشويات — Shared Data Store v4.0 (Supabase + localStorage fallback)
(function (global) {
  'use strict';
  ...
  global.MenuStore = {
    ...
    async init() {
      ...
      this._initPromise = (async () => {
        if (typeof sb !== 'undefined') {
          try {
            const { data, error } = await sb.from('menu_data').select('*').single();
            if (data && data.restaurant && data.restaurant.name_ar) {
              this._data = {
              ...
              await sb.from('menu_data').upsert({
                id: 1,
    ...
    // Real-time: listen for order changes (admin)
    onOrdersSnapshot(callback) {
      if (typeof sb === 'undefined') {
        setInterval(() => {
          try {
            const orders = JSON.parse(localStorage.getItem(KEYS.ORDERS)) || [];
            if (JSON.stringify(orders) !== JSON.stringify(this._orders)) {
              this._orders = orders; callback(orders);
            }
          } catch (e) {}
        }, 3000);
        return;
      }
      this._ordersUnsub = sb
        .channel('orders-live')
        .on('postgres_changes',
          { event: '*', schema: 'public', table: 'orders' },
          async () => {
            const { data } = await sb.from('orders').select('*').order('created_at', { ascending: false });
            this._orders = (data || []).map(r => ({
              id: r.order_id, customer_name: r.customer_name,
              customer_phone: r.customer_phone, order_type: r.order_type,
              table: r.table_number, address: r.address, notes: r.notes,
              payment_method: r.payment_method, items: r.items || [],
              subtotal: r.subtotal || 0, delivery_fee: r.delivery_fee || 0,
              total: r.total || 0, status: r.status || 'pending',
              created_at: r.created_at, updated_at: r.updated_at, _supaId: r.id,
            }));
    ...
    // Real-time: listen for menu data changes (customer site)
    onMenuSnapshot(callback) {
      if (typeof sb === 'undefined') return;
      this._menuUnsub = sb
        .channel('menu-live')
        .on('postgres_changes',
          { event: 'UPDATE', schema: 'public', table: 'menu_data', filter: 'id=eq.1' },
          async () => {
            const { data } = await sb.from('menu_data').select('*').single();
    ...
    unsubscribeAll() {
      if (this._ordersUnsub) { sb.removeChannel(this._ordersUnsub); this._ordersUnsub = null; }
      if (this._menuUnsub) { sb.removeChannel(this._menuUnsub); this._menuUnsub = null; }
    },
